...
RestAssured.baseURI =baseUri;
	int responseStatuscode =given().header("Content-Type","application/json").body(requestBody).when().patch(resource).then()
			.extract().statusCode();
...
public static String responseBodyExtractor(String baseUri, String resource, String requestBody) {
 ...
 String  responsebody =given().header("Content-Type","application/json").body(requestBody).when().patch(resource).then()
